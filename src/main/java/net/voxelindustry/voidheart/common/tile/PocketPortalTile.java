...
public class PocketPortalTile extends BlockEntity
{
    ...
    public boolean voidPieceInteract(PlayerEntity player, ItemStack voidPiece)
    {
        ...
        if (isInPocket)
        {
            if (tag.contains("externalPos"))
            {
                ...
                if (linkedPortal instanceof PocketPortalTile)
                {
                    ...
                    ((PocketPortalTile) linkedPortal).setLinkedPos(getPos().offset(getFacing()));
                    ((PocketPortalTile) linkedPortal).setLinkedWorld(VoidHeart.VOID_WORLD_KEY.getValue());
                    ((PocketPortalTile) linkedPortal).setLinkedFacing(getFacing());
        ...
        if (tag.contains("pocketPos"))
        {
            ...
            if (linkedPortal instanceof PocketPortalTile)
            {
                ...
                ((PocketPortalTile) linkedPortal).setLinkedPos(getPos().offset(getFacing()));
                ((PocketPortalTile) linkedPortal).setLinkedWorld(getWorld().getRegistryKey().getValue());
                ((PocketPortalTile) linkedPortal).setLinkedFacing(getFacing());
    ...
    public void breakLink()
    {
        ...
        PocketPortalTile linked = getLinkedPortal();
    ...
    private PocketPortalTile getLinkedPortal()
    {
        return (PocketPortalTile) getWorld().getServer().getWorld(getLinkedWorldKey()).getBlockEntity(getLinkedPos());
    }
}
